...
export class DrawingService {
    ...
    constructor(private socketService:SocketService, private lobbyService:LobbyService) {
        ...
        socketService.socket.on('guessed_drawing', (data) => {
            this.zone.run(() => {
                let drawing = this.findDrawing(data.uuid);
                drawing.is_guessed = true;
            });
        });

        socketService.socket.on('finished_drawing', (data) => {
            this.zone.run(() => {
                this.findDrawing(data.uuid).is_finished = true;
            });
        });
    }

    public guessDrawing(drawing:Drawing, word:string)
    {
        this.socketService.emit('guess_drawing', {
            uuid: drawing.uuid,
            word: word,
            username: this.lobbyService.currentUser.username
        })
    }

    public finishDrawing(uuid:string, word:string)
    {
        this.socketService.emit('finish_drawing', {
            uuid: uuid,
            word: word,
        })
    }
